...
import { lukso } from 'viem/chains'; // Use Lukso chain configuration
...
const client = createPublicClient({
  chain: lukso, // Lukso testnet chain configuration
  transport: http(RPC_URL),
});

// Create contract instance
const myAssetContract = getContract({
  address: SAMPLE_LSP8_ASSET,
...
async function fetchTokenIdMetadata(tokenID: string) {
  ...
  try {
    isLSP8 = await myAssetContract.read.supportsInterface([
   ...
   */
  const tokenIdMetadata = await myAssetContract.read.getDataForTokenId([
  ...
  const decodedMetadata = erc725js.decodeData([
    {
      ...
      value: tokenIdMetadata as string | { key: string; value: string | null; }[],
    },
  ]);
